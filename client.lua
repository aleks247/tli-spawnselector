...
RegisterNetEvent("tli:selector:show", function(data)
    if data then
        BuildSelector(data);
    else
        TriggerServerEvent("tli:selector:show")
    end
end)
...
function BuildLocations(data)
    ...
    if data then
        ...
        for i=1, #Settings.Locations do
            local PlayerData = QBCore.Functions.GetPlayerData()
            local coords, rot = BuildCameraProperties(vector3(PlayerData.position.x, PlayerData.position.y, PlayerData.position.z));
            results[1] = { name = "last", label = "Last Location", coords = coords, rot = rot, spawn = vector4(PlayerData.position.x, PlayerData.position.y, PlayerData.position.z, 100) };
...
RegisterNetEvent("tli-spawnselector:set")
AddEventHandler("tli-spawnselector:set" , function()
    BuildSelector('');
end)

--RegisterCommand('test', function()  BuildSelector('');end)